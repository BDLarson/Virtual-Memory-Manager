...
public class VirtualMemoryManagerV3 {
	...
	int pageFaults = 0;
	int transBytes = 0;
	int uFrame = 0; //Frames that are currently being used
	int oFrame = 0; //Frames that are old for FIFO utilization
	...
	public VirtualMemoryManagerV3(MainMemory memory, BackingStore disk, Integer pageSize) throws MemoryException {
		...
		pageTable = new PageTable(pageSize, (memory.size()/pageSize)); // Create standard pageTable
		pTable = new int[memory.size() / pageSize]; // Create the eviction table
		dTable = new int[memory.size() / pageSize]; // Create the dirtybit table
	}
   	...
   	public void writeByte(Integer fourByteBinaryString, Byte value) throws MemoryException {
		...
		if (pageTable.isValid(pageNumber) == false) {
			...
			if (uFrame == pTable.length) {
				...
				//Determine that eviction is not a dirtybit
				if (dTable[oFrame] == 0) {
					System.out.println("Evicting page " + pTable[oFrame] + " (NOT DIRTY)");
					pageTable.delete(pTable[oFrame]);
					frameNumber = oFrame;
					pTable[oFrame] = pageNumber;
					dTable[oFrame] = 0;
					oFrame++;
				} else { //Determine that eviction is standard
			...
			//Determine which page os being copied to a frame
  	 ...
  	 */
   	public Byte readByte(Integer fourByteBinaryString) throws MemoryException {
		...
		if (pageTable.isValid(pageNumber) == false) {
			...
			//Determine which page is being copied to a frame
			System.out.println("Bringing page " + pageNumber + " into frame " + frameNumber);
			pageFaults++;
			address = frameNumber * pageSize;
  	...
  	public void writeBackAllPagesToDisk() throws MemoryException {
		for (int i = 0; i < pTable.length; i++) { //iterate through pages of RAM
			if (pageTable.isValid(pTable[i])) { //Check if current page is a valid one
				...
				if (dTable[i] == 0) {
					currentFrame = pageTable.lookup(pTable[i]) * pageSize;
